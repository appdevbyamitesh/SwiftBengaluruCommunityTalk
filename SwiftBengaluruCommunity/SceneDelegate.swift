...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        // Setup Realm configuration with migration
        let config = Realm.Configuration(
            schemaVersion: 3,
            migrationBlock: { migration, oldSchemaVersion in
                if oldSchemaVersion < 3 {
                    // Example migration: rename property
                    migration.renameProperty(onType: Member.className(), from: "name", to: "fullName")
                    migration.enumerateObjects(ofType: Member.className()) { oldObject, newObject in
                        newObject?["address"] = "" // defult value
                    }
                }
            }
        )
